    ...
    def _establish_session(self):
        ...
        for _ in range(3):
            try:
                ...
                time.sleep(2)
        raise ConnectionError("Could not establish NSE session")

    def get_nifty_stocks(self):
        """Fetch all NIFTY 50 stock symbols dynamically"""
        try:
            url = f"{self.base_url}/api/equity-stockIndices?index=NIFTY 50"
            ...
            data = response.json()
            stock_list = [stock['symbol'] for stock in data['data']]
            return stock_list
        except:
            return []
...
async def analyze_all(update: Update, context: CallbackContext):
